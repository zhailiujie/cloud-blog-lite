...
async function countActiveAdmins(env: Env) {
  const row = await env.DB.prepare("SELECT COUNT(1) AS total FROM users WHERE role = 'admin' AND status = 1").first<{ total: number }>()
...
userRoutes.get('/', async (c) => {
  ...
  return c.json(ok((rows.results || []).map(toUser)))
})
...
userRoutes.post('/', async (c) => {
  ...
  const exists = await c.env.DB.prepare('SELECT id FROM users WHERE username = ? LIMIT 1').bind(username).first<{ id: string }>()
  if (exists) {
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, username, passwordHash, body?.nickname?.trim() || '', body?.avatar?.trim() || '', role, body?.status === 0 ? 0 : 1, body?.remark?.trim() || '', time, time)
    .run()
  ...
  return c.json(ok({ id, username, role, status: body?.status === 0 ? 0 : 1, createdAt: time, updatedAt: time }))
})

userRoutes.put('/:id', async (c) => {
  const id = c.req.param('id')
  const currentUser = c.get('user')
  const oldUser = await c.env.DB.prepare('SELECT id, role, status FROM users WHERE id = ? LIMIT 1').bind(id).first<{ id: string; role: UserRole; status: number }>()
  if (!oldUser) return c.json(fail('User not found', 404), 404)

  const body = await c.req.json<UpdateUserRequest>().catch(() => ({} as UpdateUserRequest))
  ...
  if (oldUser.role === 'admin' && oldUser.status === 1 && (nextRole !== 'admin' || nextStatus === 0)) {
    const activeAdmins = await countActiveAdmins(c.env)
    if (activeAdmins <= 1) {
      return c.json(fail('Cannot disable or demote the last active admin', 400), 400)
    }
  }
  ...
  await c.env.DB.prepare(
    ...
    .bind(body.nickname?.trim() || '', body.avatar?.trim() || '', nextRole, nextStatus, body.remark?.trim() || '', time, id)
    .run()
...
userRoutes.post('/:id/reset-password', async (c) => {
  ...
  const exists = await c.env.DB.prepare('SELECT id FROM users WHERE id = ? LIMIT 1').bind(id).first<{ id: string }>()
  if (!exists) return c.json(fail('User not found', 404), 404)

  const time = nowIso()
  await c.env.DB.prepare('UPDATE users SET password_hash = ?, login_error_count = 0, updated_at = ? WHERE id = ?')
...
userRoutes.delete('/:id', async (c) => {
  ...
  const user = await c.env.DB.prepare('SELECT id, role, status FROM users WHERE id = ? LIMIT 1').bind(id).first<{ id: string; role: UserRole; status: number }>()
